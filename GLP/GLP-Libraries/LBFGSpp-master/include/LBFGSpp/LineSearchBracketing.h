...
public:
    ...
    static void LineSearch(Foo& f, Scalar& fx, Vector& x, Vector& grad,
                           ...
                           const LBFGSParam<Scalar>& param, size_t xsize, ptr_bowl &parameters, sym_matrix &Qx, sym_matrix &Qstar, col_vector &xstar,col_vector &mu, col_vector &grad_x_like,col_vector &b_vec,col_vector &x_intial,blaze::DiagonalMatrix< blaze::DynamicMatrix<double> > &D,boost::scoped_ptr<double> &y,col_vector &Ax)
    {
        ...
        for( int iter = 0; iter < param.max_linesearch; iter++ )
        {
            ...
            fx = f(x, grad,xsize,parameters,Qx,Qstar,xstar,mu,grad_x_like,b_vec,x_intial,D,y,Ax);
